...
context('Actions', () => {
  ...
  it('.select() - select an option in a <select> element', () => {
    ...
    cy.get('.signup-select')
      .should('have.value', null)

    // Select option(s) with matching text content
    cy.get('.signup-select').select('Male')
    // confirm the apples were selected
    // note that each value starts with "fr-" in our HTML
    cy.get('.signup-select').should('have.value', 'M')

    // Select option(s) with matching value
    cy.get('.signup-select').select('M')
  ...
  it('.submit() - submit a form', () => {
    ...
    cy.get('.signup-form .signup-select')
      .select('M')
...
context('Location', () => {
  ...
  it('cy.location() - get window.location', () => {
    ...
    cy.location().should((location) => {
      ...
      expect(location.href).to.eq('http://127.0.0.1:8080/signup')
  ...
  it('cy.url() - get the current URL', () => {
    ...
    cy.url().should('eq', 'http://127.0.0.1:8080/signup')
  })
})
